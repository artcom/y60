# Some utilities for building Y60-related things

include(AcMake)

macro(y60_add_component COMPONENT_NAME)
    ac_add_plugin(
        y60${COMPONENT_NAME} y60/components
        ${ARGN}
    )
endmacro(y60_add_component)


macro(y60_add_jstest NAME )
  get_target_property(Y60_EXECUTABLE y60 LOCATION_${CMAKE_BUILD_TYPE}) 
  set(TESTPATH ${CMAKE_CURRENT_BINARY_DIR})
  string(REPLACE "${CMAKE_BINARY_DIR}" "${CMAKE_SOURCE_DIR}" TESTPATH ${TESTPATH})
  add_test(${NAME}
      ${Y60_EXECUTABLE} test${NAME}.tst.js ${ARGN} shaderlibrary.xml -I 
      "${TESTPATH};${CMAKE_BINARY_DIR}/lib/Release;${CMAKE_SOURCE_DIR}/y60/products/y60/;${CMAKE_SOURCE_DIR}/y60/js/;${CMAKE_SOURCE_DIR}/y60/shader/;."
  )
endmacro(y60_add_jstest ) 

macro(write_scenetest NAME)
  file(APPEND ${NAME}.tst.cmake "#generate image\n")
  file(APPEND ${NAME}.tst.cmake "execute_process(\n")
  file(APPEND ${NAME}.tst.cmake "  COMMAND ${CMAKE_BINARY_DIR}/y60/products/y60/Release/y60 -I \"${CMAKE_SOURCE_DIR}/y60/js;${CMAKE_BINARY_DIR}/lib/Release;${CMAKE_SOURCE_DIR}/y60/shader\" ${SCRIPT} ${SCENE} ${SHADERLIB} rehearsal offscreen outputimage=${TESTMODELDIR}/TestImages/${NAME} outputsuffix=${IMGSUFFIX}\n")
  file(APPEND ${NAME}.tst.cmake "  WORKING_DIRECTORY ${TESTMODELDIR}\n")
  file(APPEND ${NAME}.tst.cmake "  RESULT_VARIABLE rv\n")
  file(APPEND ${NAME}.tst.cmake ")\n")
  file(APPEND ${NAME}.tst.cmake "\n")
  
  set(COUNT 0)
  set(TESTIMG ${TESTMODELDIR}/TestImages/${NAME}${IMGSUFFIX}) 
  set(BASELINEIMG ${TESTMODELDIR}/BaselineImages/${NAME}${IMGSUFFIX})
  
  while(EXISTS ${BASELINEIMG})
    file(APPEND ${NAME}.tst.cmake "#compare image\n")
    file(APPEND ${NAME}.tst.cmake "if(rv EQUAL 0)\n")
    file(APPEND ${NAME}.tst.cmake "execute_process(\n")
    file(APPEND ${NAME}.tst.cmake "  COMMAND ${CMAKE_BINARY_DIR}/y60/products/CompareImage/Release/ac-compare-image --tolerance 0.12 --threshold 30 ${TESTIMG} ${BASELINEIMG}\n")
    file(APPEND ${NAME}.tst.cmake "  WORKING_DIRECTORY ${TESTMODELDIR}\n")
    file(APPEND ${NAME}.tst.cmake "  RESULT_VARIABLE rv\n")
    file(APPEND ${NAME}.tst.cmake ")\n")
    file(APPEND ${NAME}.tst.cmake "endif(rv EQUAL 0)\n")  
    file(APPEND ${NAME}.tst.cmake "\n")
    file(APPEND ${NAME}.tst.cmake "if(NOT rv EQUAL 0)\n")
    file(APPEND ${NAME}.tst.cmake "  message(FATAL_ERROR \"scenetest ${NAME} FAILED\")\n")
    file(APPEND ${NAME}.tst.cmake "endif(NOT rv EQUAL 0)\n")  
    math(EXPR COUNT ${COUNT}+1)
    set(TESTIMG ${TESTMODELDIR}/TestImages/${NAME}.${COUNT}${IMGSUFFIX}) 
    set(BASELINEIMG ${TESTMODELDIR}/BaselineImages/${NAME}.${COUNT}${IMGSUFFIX})
  endwhile(EXISTS ${BASELINEIMG})
  
endmacro(write_scenetest )

macro(y60_add_scenetest NAME ) 

  #generate  ${NAME}.tst.cmake (will include execute_process y60, ac-compare-image) 
  set(TESTMODELDIR ${CMAKE_SOURCE_DIR}/../testmodels)
  set(SCRIPT ${TESTMODELDIR}/${NAME}.js)
  if(NOT EXISTS ${SCRIPT})
    set(SCRIPT ${CMAKE_SOURCE_DIR}/y60/js/scenetest.js)
  endif(NOT EXISTS ${SCRIPT})
  set(SCENE ${TESTMODELDIR}/${NAME}.x60)
  if(NOT EXISTS ${SCENE})
    set(SCENE_H ${TESTMODELDIR}/${NAME}.b60)
    if(NOT EXISTS ${SCENE_H})
      set(SCENE_H "")
    endif(NOT EXISTS ${SCENE_H})  
    set(SCENE ${SCENE_H})
  endif(NOT EXISTS ${SCENE})
  set(SHADERLIB ${CMAKE_SOURCE_DIR}/y60/shader/shaderlibrary_nocg.xml)
  set(IMGSUFFIX .nocg.png)
  set(ONEVERSION 0)
  
  file(WRITE ${NAME}.tst.cmake  "# testscript for scenetest ${NAME}\n")
  file(APPEND ${NAME}.tst.cmake "#   will be called with cmake -P ${TESTMODELDIR}/${NAME}.tst.cmake\n\n")
  if(EXISTS ${TESTMODELDIR}/${NAME}.pre.cmake)
    file(APPEND ${NAME}.tst.cmake "include(${TESTMODELDIR}/${NAME}.pre.cmake)\n")
    file(APPEND ${NAME}.tst.cmake "${NAME}_pre(${CMAKE_BINARY_DIR} ${TESTMODELDIR}) \n\n")
  endif(EXISTS ${TESTMODELDIR}/${NAME}.pre.cmake)
  
  if(EXISTS ${TESTMODELDIR}/BaselineImages/${NAME}${IMGSUFFIX})
    set(ONEVERSION 1)
    file(APPEND ${NAME}.tst.cmake "message(\"*********************** NO CG *********************\")\n")
    write_scenetest( ${NAME} )
  endif(EXISTS ${TESTMODELDIR}/BaselineImages/${NAME}${IMGSUFFIX})
  set(SHADERLIB ${CMAKE_SOURCE_DIR}/y60/shader/shaderlibrary.xml)
  set(IMGSUFFIX .cg.png)
  if(EXISTS ${TESTMODELDIR}/BaselineImages/${NAME}${IMGSUFFIX})
    set(ONEVERSION 1)
    file(APPEND ${NAME}.tst.cmake "\n\nmessage(\"*********************** CG *********************\")\n")
    write_scenetest( ${NAME} )
  endif(EXISTS ${TESTMODELDIR}/BaselineImages/${NAME}${IMGSUFFIX})

  if(${ONEVERSION} EQUAL 0)  
    file(APPEND ${NAME}.tst.cmake "  message( \"SKIPPED. (No BaselineImages found for scenetest ${NAME}.)\")\n")
  endif(${ONEVERSION} EQUAL 0)  
  file(APPEND ${NAME}.tst.cmake "\n") 
  
  # register the created cmake-file as test
  add_test(${NAME}
    cmake -P ${TESTMODELDIR}/${NAME}.tst.cmake
  )
endmacro(y60_add_scenetest ) 

