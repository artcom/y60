...
double TTF_Kerning(TTF_Font * theFont, const Uint16 theFirstCharacter, const Uint16 theSecondCharacter) {
    ...
    use_kerning = FT_HAS_KERNING( theFont->face ) && theFont->kerning;
	
	if ( use_kerning && myFirstIndex && mySecondIndex ) {
		FT_Vector delta; 
		FT_Get_Kerning( theFont->face, myFirstIndex, mySecondIndex, ft_kerning_default, &delta ); 
		myKerning = (double) (delta.x >> 6);
	}
	return myKerning;
}
...
void TTF_SetFontHinting( TTF_Font* font, int hinting )
{
	...
	else if (hinting == TTF_HINTING_NORMAL)
		font->hinting = 0;
	if (hinting >= TTF_HINTING_AUTO)
        font->hinting |= FT_LOAD_FORCE_AUTOHINT;
	Flush_Cache( font );
}
