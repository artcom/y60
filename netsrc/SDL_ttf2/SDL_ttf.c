...
/* $Id: SDL_ttf.c,v 1.13 2005/01/12 20:06:35 christian Exp $ */
...
/* [ART+COM Patch]: Added additional members for unfitted positioning */
...
static FT_Error Load_Glyph( TTF_Font* font, Uint16 ch, c_glyph* cached, int want )
{
	...
	/* [ART+COM Patch]: Adapted metric calculation from Photoshop */
...
Uint16 *LATIN1_to_UNICODE(Uint16 *unicode, const char *text, int len)
{
	int i;
...
int TTF_SizeUNICODE(TTF_Font *font, const Uint16 *text, int *w, int *h) {
	...
	for (ch = text; *ch; ++ch) {
        ...
        /* Add kerning [ART+COM Patch] */
        xstart += getKerning(font, prev_index, glyph->index);
...
double TTF_Kerning(TTF_Font * theFont, const Uint16 theFirstCharacter, const Uint16 theSecondCharacter) {
	...
	error = Find_Glyph(theFont, theFirstCharacter, CACHED_METRICS|CACHED_BITMAP);
	if (error) {
        return 0.0;
    }

    myFirstIndex = theFont->current->index;

    error = Find_Glyph(theFont, theSecondCharacter, CACHED_METRICS|CACHED_BITMAP);
	if (error) {
        return 0.0;
    }

    mySecondIndex = theFont->current->index;

    return getKerning(theFont, myFirstIndex, mySecondIndex);
}
				...
				const Uint16 *text, SDL_Color fg)
{
	...
	for( ch=text; *ch; ++ch ) {
        ...
        /* Save minx of first glyph for precise line positioning [ART+COM Patch] */
		if (ch == text) {
		    TTF_current_line_minx = glyph->minx;
		}
