...
public final class CBuilderXProjectWriter
  ...
  public void writeProject(final File fileName,
    ...
    if (compilerConfig == null) {
      throw new BuildException(
          "Unable to generate C++ BuilderX project when gcc is not used.");
    }
    ...
    propertyWriter.write("build.platform", "win32.default", "MinGW");
    propertyWriter.write("build.platform", "win32.enabled", "1");
    //propertyWriter.write("cbproject", "lastnodeid", "852");
    ...
    if ("dllproject".equals(buildType)) {
      ...
      propertyWriter.write("win32b.bcc32", "option.tWD.enabled", "1");
      propertyWriter.write("win32b.bcc32", "option.tWM.enabled", "1");
      propertyWriter.write("win32b.bcc32", "option.tWR.enabled", "1");
      propertyWriter.write("win32b.ilink32", "option.Gi.enabled", "1");
      propertyWriter.write("win32b.ilink32", "option.Tpd.enabled", "1");
      propertyWriter.write("win32b.ilink32", "option.Tpe.enabled", "0");
      propertyWriter.write("win32b.ilink32", "option.aa.enabled", "1");
      propertyWriter.write("win32b.ilink32", "param.objfiles.1", "c0d32.obj");
      propertyWriter.write("win32b.implib", "enabled", "0");
      ...
      propertyWriter.write("win32b.bcc32", "option.tWC.enabled", "1");
      propertyWriter.write("win32b.ilink32", "option.Tpd.enabled", "0");
      propertyWriter.write("win32b.ilink32", "option.Tpe.enabled", "1");
      propertyWriter.write("win32b.ilink32", "option.aa.enabled", "0");
      propertyWriter.write("win32b.ilink32", "option.ap.enabled", "1");
      propertyWriter.write("win32b.ilink32", "param.objfiles.1", "c0x32.obj");
      propertyWriter.write("win32b.implib", "enabled", "0");
    }
  ...
  private void writeCompileOptions(final String baseDir,
    ...
    String gnuCompile = "linux.Debug_Build.gnuc++.g++compile";

    File[] includePath = compilerConfig.getIncludePath();
    int includeIndex = 1;
    writer.write(gnuCompile,
    ...
    for (int i = 0; i < includePath.length; i++) {
      writer.write(gnuCompile,
                   "option.I.arg." + (includeIndex++),
                   CUtil.getRelativePath(baseDir, includePath[i]));
    }
    ...
    for (int i = 0; i < preArgs.length; i++) {
      if (preArgs[i].startsWith("-D")) {
        writer.write(gnuCompile,
                     "option.D_MACRO_VALUE.arg."
                     + (defineIndex++), preArgs[i]
                     .substring(2));
  ...
  private void
    ...
    if (linkTarget != null) {
      ...
      if (config instanceof CommandLineLinkerConfiguration) {
        ...
        String gnuLink = "linux.Debug_Build.gnuc++.g++link";

        writeLinkArgs(writer, gnuLink, linkConfig.getPreArguments());
        writeLinkArgs(writer, gnuLink, linkConfig.getEndArguments());
      }
    }
  }

  /**
   * Writes linker options to project file.
   *
   * @param writer PropertyWriter property writer
   * @param gnuLink String linker identifier
   * @param preArgs String[] linker arguments
   * @throws SAXException thrown if unable to write option
   */
  private void writeLinkArgs(final PropertyWriter writer,
                             final String gnuLink,
    ...
    for (int i = 0; i < preArgs.length; i++) {
      if (preArgs[i].startsWith("-o")) {
        ...
        //  any others
        //
        writer.write(gnuLink, "option." + preArgs[i].substring(1) + ".enabled",
                     "1");
      }
    }
    if (objnameIndex > 1) {
