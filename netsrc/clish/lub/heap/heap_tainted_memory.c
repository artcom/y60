...
extern function_t taskDestroy; /* fiddle a reference to the function */
        /* 
         * VxWorks taskDestroy() relies on being able to access free memory.
         * It calls objFree() followed by objTerminate() !!!!
         * So we have to perform a crufty hack to avoid this mistake blowing
         * us out of the water...
         */
        if(LUB_HEAP_TAINT_FREE == type)
        {
            ...
            for(i = 0; i < 10; i++)
            {
                int delta = (unsigned)frame.backtrace[i] - (unsigned)&taskDestroy;
