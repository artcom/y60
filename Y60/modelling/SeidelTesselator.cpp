...
namespace y60 {
    ...
    int 
    SeidelTesselator::inside_polygon(trap_t * t)
    {
      int rseg = t->rseg;
    ...
    int 
        SeidelTesselator::new_chain_element()
    {
      return ++chain_idx;
    }
    ...
    double 
        SeidelTesselator::get_angle(point_t * vp0, point_t * vpnext, point_t * vp1)
    {
      point_t v0, v1;
    ...
    int 
        SeidelTesselator::get_vertex_positions(int v0, int v1, int * ip, int * iq)
    {
     ...
     */
    int 
        SeidelTesselator::make_new_monotone_poly(int mcur, int v0, int v1)
    {
      int p, q, ip, iq;
      int mnew = newmon();
      int i, j, nf0, nf1;
    ...
    int 
        SeidelTesselator::monotonate_trapezoids(int n)
    {
      register int i;
      int tr_start;
    ...
    int 
        SeidelTesselator::traverse_polygon(int mcur, int trnum, int from, int dir)
    {
      trap_t *t = &tr[trnum];
      int mnew;
      int v0, v1;
    ...
    /* Return the maximum of the two points into the yval structure */
     int SeidelTesselator::_max(point_t *yval, point_t *v0, point_t *v1)
    {
      if (v0->y > v1->y + C_EPS)
        *yval = *v0;
    ...
    /* Return the minimum of the two points into the yval structure */
     int SeidelTesselator::_min(point_t *yval, point_t *v0, point_t *v1)
    {
      if (v0->y < v1->y - C_EPS)
        *yval = *v0;
      else if (FP_EQUAL(v0->y, v1->y))
    ...
    int SeidelTesselator::_greater_than(point_t *v0, point_t *v1)
    {
      if (v0->y > v1->y + C_EPS)
        return TRUE;
    ...
    int SeidelTesselator::_equal_to(point_t * v0, point_t *v1)
    {
      return (FP_EQUAL(v0->y, v1->y) && FP_EQUAL(v0->x, v1->x));
    }
    
    int SeidelTesselator::_greater_than_equal_to(point_t *v0, point_t *v1)
    {
      if (v0->y > v1->y + C_EPS)
        return TRUE;
      else if (v0->y < v1->y - C_EPS)
    ...
    int SeidelTesselator::_less_than(point_t *v0, point_t *v1)
    {
      if (v0->y < v1->y - C_EPS)
        return TRUE;
      else if (v0->y > v1->y + C_EPS)
     ...
     */
     ...
     int SeidelTesselator::init_query_structure(int segnum)
    {
     ...
     */
     ...
     int SeidelTesselator::is_left_of(int segnum, point_t * v)
    {
      segment_t *s = &seg[segnum];
      double area;
    ...
    /* whether the segment which shares this endpoint is already inserted */
     ...
     int SeidelTesselator::inserted(int segnum, int whichpt)
    {
      if (whichpt == FIRSTPT)
    ...
    int SeidelTesselator::locate_endpoint(point_t *v, point_t *vo, int r)
    {
      node_t *rptr = &qs[r];
     ...
     */
     ...
     int SeidelTesselator::merge_trapezoids(int segnum, int tfirst, int tlast, int side)
    {
      int t, tnext, cond;
      int ptnext;
     ...
     */
     ...
     int SeidelTesselator::add_segment(int segnum)
    {
      segment_t s;
     ...
     */
     int SeidelTesselator::find_new_roots(int segnum)
    {
      segment_t *s = &seg[segnum];
    ...
    int SeidelTesselator::construct_trapezoids(int nseg)
    {
      register int i;
      int root, h;
