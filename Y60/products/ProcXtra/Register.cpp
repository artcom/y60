 ...
 *     CProc           <name of the class you defined>
 *
 ...
 ****************************************************************************/
BEGIN_DEFINE_CLASS_INTERFACE(CProc, IMoaRegister)
...
static char msgTable[] = {
	"xtra Proc -- Allow Director to start and control external processes.\n"
	"* ProcStart string directory, string application -- Start external application; returns process handle.\n"
	"* ProcReparent integer pid, string viewer, string director -- Reparent window 'viewer' to window 'director'.\n"
	"* ProcTerminate integer pid, integer waitTime -- Terminate process. If waitTime[ms] is -1 then wait indefinitively.\n"
	"* ProcWait integer pid, integer waitTime -- Wait for process to terminate. If waitTime[ms] is -1 then wait indefinitively.\n"
	};
...
/* ---------------------------------------------------- MoaCreate_CProc */
STDMETHODIMP MoaCreate_CProc(CProc FAR * This)
{
	UNUSED(This);
...
/* --------------------------------------------------- MoaDestroy_CProc */
STDMETHODIMP_(void) MoaDestroy_CProc(CProc FAR * This)
{
	UNUSED(This);
...
/* ---------------------------------- CProc_IMoaRegister Create/Destroy */
CProc_IMoaRegister::CProc_IMoaRegister(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
CProc_IMoaRegister::~CProc_IMoaRegister() {}
...
/* ------------------------------------------- CProc_IMoaRegister::Register */
STDMETHODIMP_(MoaError) CProc_IMoaRegister::Register(PIMoaCache pCache,
	PIMoaDict pXtraDict)
{
	...
	err = pCache->AddRegistryEntry(pXtraDict, &CLSID_CProcScript, &IID_IMoaMmXScript, &pRegDict);
...
//		sprintf( buffer, "CProc::Register:\nxtra registered\n");
//		MessageBox( 0, buffer, "Xtra Proc - Info", MB_ICONINFORMATION );
	} else {
