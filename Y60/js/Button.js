...
function TripleImageToggleButton(theScene, theId, theSources,
                         thePosition, theStyle, theParent, theButtonGroup, theButtonsToActivate) {
    var Base      = {};
    var Public    = this;
    var Protected = {};
    ButtonBase(Public, Protected, theScene, theId,
    ...
    Public.isActive = function(theButton) {
        return Protected.active;   
    }
    
    Public.setActive = function(theFlag) {
        if (!Public.enabled) {
            return;   
        }
        if (theFlag) {
            for(i = 0; i < _myButtonGroup.length; i++) {
                _myButtonGroup[i].setActive(false);
            }
            if(_myButtonsToActivate) {
                for(i = 0; i < _myButtonsToActivate.length; i++) {
                    _myButtonsToActivate[i].setActive(true);
                }   
            }
            Public.color = Public.style.selectedColor;
            Public.setImage(theSources[1]);
        } else {
            if(!Protected.isPressed) {
                Public.color = Public.style.color;
                Public.setImage(theSources[0]);
            }
            
        }
        Protected.active = theFlag;
    }
    ...
    Base.enabled = Public.enabled;
    Public.enabled setter = function(theFlag) {
        if (Protected.enabled != theFlag) {
            Protected.enabled = theFlag;
            if (!theFlag) {
                 Public.color = Public.style.color;
                 Public.setImage(theSources[2]);
            } else {
                 Public.color = Public.style.color;
                 Public.setImage(theSources[2]);
            }
    ...
    var _myButtonGroup = theButtonGroup;
    var _myButtonsToActivate = theButtonsToActivate;
}
