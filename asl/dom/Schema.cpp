...
dom::Schema::getAttributeDeclaration(const NodePtr theElementType, const DOMString & theAttributeName) const {
    ...
    if (theElementType->nodeName() == XS_COMPLEXTYPE) {
        DB(AC_TRACE << "getAttributeDeclaration: childNodeByAttribute complexType <xs:attribute name='"<<theAttributeName<<"'>"<<endl);
        NodePtr myAttrDecl = theElementType->childNodeByAttribute(XS_ATTRIBUTE,ATTR_NAME,theAttributeName);
        if (myAttrDecl) {
            DB(AC_TRACE << "getAttributeDeclaration: childNodeByAttribute : returning myAttrDecl = "<<*myAttrDecl<<endl);
            return myAttrDecl;
        }
        NodePtr myContent = theElementType->childNode(XS_SIMPLECONTENT);
        if (!myContent) {
            DB(AC_TRACE << "getAttributeDeclaration: childNodeByAttribute : no simpleContent= "<<endl);
            myContent = theElementType->childNode(XS_COMPLEXCONTENT);
        }
        if (myContent) {
            DB(AC_TRACE << "getAttributeDeclaration: childNodeByAttribute : myContent= "<<*myContent<<endl);
            ...
            if (myExOrRes) {
                DB(AC_TRACE << "getAttributeDeclaration: childNodeByAttribute : extension/restriction = "<<*myExOrRes<<endl);
