...
LockUnitTest::run() {
    ...
    try {
        ...
        _myLock.readlock();
        for (int i = 0; i < 100;++i) {
            cerr << "HIGH_LOCKED_LOOP START " << get_cycles()-cycles<< endl;cycles=get_cycles();
            for (int i = 0; i < 100000;++i);
            cerr << "HIGH_LOCKED_LOOP END " << get_cycles()-cycles<< endl;cycles=get_cycles();
             _myLock.readunlock();
            cerr << "HIGH_LOOP START " << get_cycles()-cycles<< endl;cycles=get_cycles();
            //for (int i = 0; i < 100000;++i);
            ...
            cerr << "HIGH_LOOP END " << get_cycles()-cycles<< endl;cycles=get_cycles();
            _myLock.readlock();
        };
        _myLock.readunlock();
