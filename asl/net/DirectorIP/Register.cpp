 ...
 *     CIPClient       <name of the class you defined>
 *
 ...
 ****************************************************************************/
BEGIN_DEFINE_CLASS_INTERFACE(CIPClient, IMoaRegister)
...
static char msgTable[] = {
	"xtra IPClient -- IP-based communication between director and another program.\n"
	"* IPClientTCPConnect string serverName, integer serverPort -- connect to a remote computer via TCP/IP.\n"	
	"* IPClientUDPConnect string remoteName, integer remotePort, string localName, integer localPort -- connect to a remote computer via UDP/IP.\n"
	"* IPClientReceiveMsg -- Non-blocking read. Returns one message received. Message delimiter is newline.\n"
    "* IPClientGetStatus -- Returns result code of last ReceiveMsg call.\n"
	"* IPClientSendMsg string msg -- Send message.\n"
	"* IPClientDisconnect\n"
	"* IPClientGetErrString integer returnCode -- Get a string from a returnCode.\n"
	"* IPClientUDPOpen string localName, integer localPort -- Opens a local port to listen to UDP messages.\n"
	"* IPClientReceiveUDPMsg -- Non-blocking read from the opened UDP port. Returns one message received. Message delimiter is newline.\n"
	"* IPClientGetUDPMsgOrigin -- Returns the Host from which the last UDP message came from.\n"
	"* IPClientSendUDPMsg string remoteHost, integer remotePort, string Msg -- Sends a UDP message.\n"
	};
...
/* ---------------------------------------------------- MoaCreate_CIPClient */
STDMETHODIMP MoaCreate_CIPClient(CIPClient FAR * This)
{
	UNUSED(This);
...
/* --------------------------------------------------- MoaDestroy_CIPClient */
STDMETHODIMP_(void) MoaDestroy_CIPClient(CIPClient FAR * This)
{
	UNUSED(This);
...
/* ---------------------------------- CIPClient_IMoaRegister Create/Destroy */
CIPClient_IMoaRegister::CIPClient_IMoaRegister(MoaError FAR * pErr)
	{ *pErr = (kMoaErr_NoErr); }
CIPClient_IMoaRegister::~CIPClient_IMoaRegister() {}
...
/* ------------------------------------------- CIPClient_IMoaRegister::Register */
STDMETHODIMP_(MoaError) CIPClient_IMoaRegister::Register(PIMoaCache pCache,
	PIMoaDict pXtraDict)
{
	...
	err = pCache->AddRegistryEntry(pXtraDict, &CLSID_CIPClientScript, &IID_IMoaMmXScript, &pRegDict);
...
//		sprintf( buffer, "CIPClient::Register:\nxtra registered\n");
//		MessageBox( 0, buffer, "Xtra IPClient - Info", MB_ICONINFORMATION );
	} else {
